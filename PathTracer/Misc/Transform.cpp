...
Matrix invert(const Matrix& M)
{
    ...
    if (D) {
        ...
        std::cerr << "WARNING: Matrix has no inverse!" << std::endl;
    }
    return ret;
}

Vector3 change_coord_system_point(Vector3 v, Vector3 b1, Vector3 b2, Vector3 b3, Vector3 origin)
{
...
Vector3 change_coord_system_dir(Vector3 v, Vector3 b1, Vector3 b2, Vector3 b3, Vector3 origin)
{
    Matrix m = base_change_matrix(b1, b2, b3, origin);
    m = invert(m);
    return mult_dir(m, v);
...
{
    float v[4] = { vec.x, vec.y, vec.z, 0.0f };
    float ret[4];
    for (int i = 0; i < 4; i++)
    {
        ret[i] = 0;
